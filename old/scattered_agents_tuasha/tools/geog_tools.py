"""geog_syllabus_lookup — Look up NCCA Geography learning outcomes."""
...
async def lookup_geog_lo(topic: str, level: str = "lc_hl", language: str = "en", limit: int = 10) -> list[dict[str, Any]]:
    try:
        from cianfhoghlaim.lancedb.search import semantic_search
        results = await semantic_search(table=f"oideachais.lc.geography.{level}_{language}", query=topic, embed_model=_resolve_embedder(), top_k=limit)
        return [{"lo_code": r.get("metadata", {}).get("lo_code", ""), "topic": topic, "score": r.get("score", 0.0)} for r in results]
    except Exception:
        return []


async def lookup_geog_paper(topic: str, level: str = "lc_hl", year: int | None = None, limit: int = 5) -> list[dict[str, Any]]:
    try:
        import duckdb
        con = duckdb.connect("./data/geography.duckdb", read_only=True)
        where = ["subject = 'geography'", f"level = '{level}'", f"text ILIKE '%{topic}%'"]
        if year is not None:
            where.append(f"year = {year}")
        query = f"SELECT item_id, lo_code, text, marks, year, source_pdf FROM geog_paper_items WHERE {' AND '.join(where)} LIMIT {limit}"
...
async def lookup_geog_marking_scheme(lo_code: str) -> dict[str, Any]:
    try:
        import duckdb
        con = duckdb.connect("./data/geography.duckdb", read_only=True)
        row = con.execute("SELECT lo_code, marks_per_step, text_en, text_ga, source_pdf FROM geog_marking_schemes WHERE lo_code = ? LIMIT 1", [lo_code]).fetchone()
        if row is None:
...
async def score_geog_response(item_id: str, student_response: str, response_format: str = "text", time_taken_seconds: int = 0, hints_used: int = 0) -> dict[str, Any]:
    try:
        from cianfhoghlaim.baml_client import b
        import duckdb
        con = duckdb.connect("./data/geography.duckdb", read_only=True)
        item_row = con.execute("SELECT item_json FROM geog_quest_items WHERE id = ? LIMIT 1", [item_id]).fetchone()
        if item_row is None:
