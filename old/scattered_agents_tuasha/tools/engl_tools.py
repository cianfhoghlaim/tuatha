...
"""engl_syllabus_lookup — Look up NCCA English learning outcomes."""
from __future__ import annotations

from typing import Any


async def lookup_engl_lo(topic: str, level: str = "lc_hl", language: str = "en", limit: int = 10) -> list[dict[str, Any]]:
    try:
        from cianfhoghlaim.lancedb.search import semantic_search
        results = await semantic_search(table=f"oideachais.lc.english.{level}_{language}", query=topic, embed_model=_resolve_embedder(), top_k=limit)
        return [{"lo_code": r.get("metadata", {}).get("lo_code", ""), "topic": topic, "score": r.get("score", 0.0)} for r in results]
    except Exception:
        return []


async def lookup_engl_paper(topic: str, level: str = "lc_hl", year: int | None = None, limit: int = 5) -> list[dict[str, Any]]:
    try:
        import duckdb
        con = duckdb.connect("./data/english.duckdb", read_only=True)
        where = ["subject = 'english'", f"level = '{level}'", f"text ILIKE '%{topic}%'"]
        if year is not None:
            where.append(f"year = {year}")
        query = f"SELECT item_id, lo_code, text, marks, year, source_pdf FROM engl_paper_items WHERE {' AND '.join(where)} LIMIT {limit}"
...
async def lookup_engl_marking_scheme(lo_code: str) -> dict[str, Any]:
    try:
        import duckdb
        con = duckdb.connect("./data/english.duckdb", read_only=True)
        row = con.execute("SELECT lo_code, marks_per_step, text_en, text_ga, source_pdf FROM engl_marking_schemes WHERE lo_code = ? LIMIT 1", [lo_code]).fetchone()
        if row is None:
...
async def generate_engl_item(lo_code: str, difficulty: int, evidence: dict[str, Any], level: str = "lc_hl", topic: str = "") -> dict[str, Any]:
    """`evidence` (`EnglEvidenceLink`-shaped: source_pdf, source_page,
    ...
    try:
        ...
        import uuid
        item = b.GenerateEnglFormativeItem(lo_code=lo_code, difficulty=difficulty, level=level, topic=topic, evidence=evidence)
...
async def score_engl_response(item_id: str, student_response: str, response_format: str = "text", time_taken_seconds: int = 0, hints_used: int = 0) -> dict[str, Any]:
    try:
        from cianfhoghlaim.baml_client import b
        import duckdb
        con = duckdb.connect("./data/english.duckdb", read_only=True)
        item_row = con.execute("SELECT item_json FROM engl_quest_items WHERE id = ? LIMIT 1", [item_id]).fetchone()
        if item_row is None:
