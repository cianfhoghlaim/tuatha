...
def xmen_raw_scenes(
    ...
    """Ingest X-Men: Evolution episode stills captured by the Hermes Agent
    computer-use client (Phase-2 path, per the 2026-08-27 change).

    The Hermes client reads
    `s3://cianfhoghlaim-tuatha-raw/xmen/<episode>/keyframes/frame-NN.jpg`
    and writes a per-run `manifest.jsonl`. This asset records the count.
    """
    if config.source != "xmen_evolution":
        return {"skipped": True}
    manifest = Path(config.manifest_path)
    if not manifest.exists():
        return {"skipped": True}
    scenes = sum(1 for _ in manifest.open())
    context.add_metadata({"scenes": scenes, "run_id": config.run_id})
    return {"run_id": config.run_id, "scenes": scenes}
...
@asset(group_name="tuatha_embed", compute_kind="cocoindex", deps=["xmen_raw_scenes"])
def xmen_scenes_embedded(context: AssetExecutionContext) -> dict[str, Any]:
    """Drive the xmen_scenes_app CocoIndex v1 flow.

    Runs `cocoindex update tuatha_xmen_scenes` and returns the row count
    in the Lance table cianfhoghlaim.tuatha.xmen.scenes.
    """
    import subprocess

    proc = subprocess.run(
        ["cocoindex", "update", "tuatha_xmen_scenes"],
        capture_output=True,
        text=True,
        cwd=_repo_root(),
    )
